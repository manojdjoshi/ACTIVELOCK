...
Send to ActiveLock@ActiveLock.com";
  ...
  trialRemaining = mfc.RemainingTrialRuns ();  // ******************************most probably Days
  trialTotal = mfc.TrialLength ();
  ...
  DDX_Text(pDX, IDC_LIC_TRIALREMAINING, 		 trialRemaining);
  DDX_Text(pDX, IDC_LIC_TRIALOUTOF, 	trialTotal);
  if(mfc.RegStatus()){
  }else{
  }
