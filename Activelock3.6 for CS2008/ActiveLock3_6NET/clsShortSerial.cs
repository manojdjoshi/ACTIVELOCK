...
internal class clsShortSerial
{
	...
	//*******************************************************************************
	public string GenerateKey(ref string sAppNameVersionPassword, ref string sHDDfirmwareSerial)
	{
		int lChar = 0;
		int lCount = 0;
		string sMD5 = null;
		...
		sMD5 = modMD5.ComputeHash(ref Strings.UCase(sAppNameVersionPassword) + sHDDfirmwareSerial);
		...
		sKey = "";
		// doing only 8-digit derial number for ease of use
		for (lCount = 1; lCount <= 8; lCount++) {
			lChar = (int)"&H" + Strings.Mid(sMD5, (lCount * 2) - 1, 2) % 10;
			//doing only 10 digits instead of say 32
			sKey = sKey + Strings.Mid(VALID_CHARS, lChar + 1, 1);
		}
	...
	public bool IsKeyOK(ref string sKey, ref string sAppNameVersionPassword, ref string sHDDfirmwareSerial)
	{
		int lChar = 0;
		int lCount = 0;
		//Dim oMD5        As CMD5
		string sMD5 = null;
		string sTestKey = null;
		...
		for (lCount = 1; lCount <= 8; lCount++) {
			lChar = (int)"&H" + Strings.Mid(sMD5, (lCount * 2) - 1, 2) % 10;
			//32
			sTestKey = sTestKey + Strings.Mid(VALID_CHARS, lChar + 1, 1);
		}

		// Check for equality
		return (sTestKey == sKey);
	}
}
