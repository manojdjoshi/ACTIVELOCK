using Microsoft.VisualBasic;
using Microsoft.VisualBasic.Compatibility;
using System;
using System.Collections;
using System.Data;
using System.Diagnostics;
using System.Drawing;
using System.Windows.Forms;
using System.Runtime.InteropServices;
using System.Globalization;
using System.Text;

//Ported from C# and modified as needed 11/11/06 By Dwain Snickles 
//Left Out Windows 64 Bit.
namespace CWindows
{

	public enum OSPlatformId
	{
		Win32s = 0,
		Win32Windows = 1,
		Win32NT = 2,
		WinCE = 3
	}
	//OSPlatformId

	public enum OSBuildNumber
	{
		None = 0,
		Win2000SP4 = 2195,
		WinXPSP2 = 2600,
		Win2003SP1 = 3790
	}
	//OSBuildNumber

	[Flags()]
	public enum OSSuites
	{
		None = 0,
		SmallBusiness = 0x1,
		Enterprise = 0x2,
		BackOffice = 0x4,
		Communications = 0x8,
		Terminal = 0x10,
		SmallBusinessRestricted = 0x20,
		EmbeddedNT = 0x40,
		Datacenter = 0x80,
		SingleUserTS = 0x100,
		Personal = 0x200,
		Blade = 0x400,
		EmbeddedRestricted = 0x800
	}
	//OSSuites

	public enum OSProductType
	{
		Invalid = 0,
		Workstation = 1,
		DomainController = 2,
		Server = 3
	}
	//OSProductType

	public enum OSVersion
	{
		Win32s,
		Win95,
		Win98,
		WinME,
		WinNT351,
		WinNT4,
		Win2000,
		WinXP,
		Win2003,
		WinCE,
		WinVista,
		Unknown
	}
	//OSVersion

	//-----------------------------------------------------------------------------
	// OSVersionInfo
	public class OSVersionInfo
	{
		//Implements ICloneable 'ToDo: Add Implements Clauses for implementation methods of these interface(s)

		//-----------------------------------------------------------------------------
		// Constants
		private class MajorVersionConst
		{
				// TODO: check
			public const int Win32s = 0;
			public const int Win95 = 4;
			public const int Win98 = 4;
			public const int WinME = 4;
			public const int WinNT351 = 3;
			public const int WinNT4 = 4;
			public const int WinNT5 = 5;
			public const int WinNT6 = 6;
			public const int Win2000 = WinNT5;
			public const int WinXP = WinNT5;
			public const int Win2003 = WinNT5;

			public const int WinVista = WinNT6;
			private MajorVersionConst()
			{
			}
			//New
		}
		//MajorVersionConst 

		private class MinorVersionConst
		{
				// TODO: check
			public const int Win32s = 0;
			public const int Win95 = 0;
			public const int Win98 = 10;
			public const int WinME = 90;
			public const int WinNT351 = 51;
			public const int WinNT4 = 0;
			public const int Win2000 = 0;
			public const int WinXP = 1;
			public const int Win2003 = 2;

			public const int WinVista = 0;
			private MinorVersionConst()
			{
			}
			//New
		}
		//MinorVersionConst 
		//-----------------------------------------------------------------------------
		// Static Fields
		private static OSVersionInfo _Win32s = new OSVersionInfo(CWindows.OSPlatformId.Win32s, MajorVersionConst.Win32s, MinorVersionConst.Win32s, true);
		private static OSVersionInfo _Win95 = new OSVersionInfo(CWindows.OSPlatformId.Win32Windows, MajorVersionConst.Win95, MinorVersionConst.Win95, true);
		private static OSVersionInfo _Win98 = new OSVersionInfo(CWindows.OSPlatformId.Win32Windows, MajorVersionConst.Win98, MinorVersionConst.Win98, true);
		private static OSVersionInfo _WinME = new OSVersionInfo(CWindows.OSPlatformId.Win32Windows, MajorVersionConst.WinME, MinorVersionConst.WinME, true);
		private static OSVersionInfo _WinNT351 = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.WinNT351, MinorVersionConst.WinNT351, true);
		private static OSVersionInfo _WinNT4 = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.WinNT4, MinorVersionConst.WinNT4, true);
		private static OSVersionInfo _Win2000 = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.Win2000, MinorVersionConst.Win2000, true);
		private static OSVersionInfo _WinXP = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.WinXP, MinorVersionConst.WinXP, true);
		private static OSVersionInfo _Win2003 = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.Win2003, MinorVersionConst.Win2003, true);
		private static OSVersionInfo _WinVista = new OSVersionInfo(CWindows.OSPlatformId.Win32NT, MajorVersionConst.WinVista, MinorVersionConst.WinVista, true);
			// TODO: WinCE version
		private static OSVersionInfo _WinCE = new OSVersionInfo(CWindows.OSPlatformId.WinCE, true);
		//-----------------------------------------------------------------------------
		// Static Properties
		private static OSVersionInfo Win32s {
			get { return _Win32s; }
		}

		private static OSVersionInfo Win95 {
			get { return _Win95; }
		}

		private static OSVersionInfo Win98 {
			get { return _Win98; }
		}

		private static OSVersionInfo WinME {
			get { return _WinME; }
		}

		private static OSVersionInfo WinNT351 {
			get { return _WinNT351; }
		}

		private static OSVersionInfo WinNT4 {
			get { return _WinNT4; }
		}

		private static OSVersionInfo Win2000 {
			get { return _Win2000; }
		}

		private static OSVersionInfo WinXP {
			get { return _WinXP; }
		}

		private static OSVersionInfo Win2003 {
			get { return _Win2003; }
		}

		private static OSVersionInfo WinVista {
			get { return _WinVista; }
		}

		private static OSVersionInfo WinCE {
			get { return _WinCE; }
		}
		//-----------------------------------------------------------------------------
		// Static methods
		public static OSVersionInfo GetOSVersionInfo(OSVersion v)
		{
			switch (v) {
				case CWindows.OSVersion.Win32s:
					return Win32s;
				case CWindows.OSVersion.Win95:
					return Win95;
				case CWindows.OSVersion.Win98:
					return Win98;
				case CWindows.OSVersion.WinME:
					return WinME;
				case CWindows.OSVersion.WinNT351:
					return WinNT351;
				case CWindows.OSVersion.WinNT4:
					return WinNT4;
				case CWindows.OSVersion.Win2000:
					return Win2000;
				case CWindows.OSVersion.WinXP:
					return WinXP;
				case CWindows.OSVersion.Win2003:
					return Win2003;
				case CWindows.OSVersion.WinVista:
					return WinVista;
				case CWindows.OSVersion.WinCE:
					return WinCE;
				default:
					return null;
			}
		}
		//GetOSVersionInfo

		//-----------------------------------------------------------------------------
		// Fields
		// normal fields

		private OSPlatformId _OSPlatformId;
		private int _MajorVersion = -1;
		private int _MinorVersion = -1;
		private int _BuildNumber = -1;
		//		private int _PlatformId;

		private string _CSDVersion = string.Empty;
		// extended fields
		private OSSuites _OSSuiteFlags;

		private OSProductType _OSProductType;
		private Int16 _ServicePackMajor = -1;
		private Int16 _ServicePackMinor = -1;
		//		private UInt16 _SuiteMask;
		//		private byte _ProductType;

		private byte _Reserved;
		// state fields
		private bool _Locked = false;
		private bool _ExtendedPropertiesAreSet = false;
		//-----------------------------------------------------------------------------
		// Normal Properties

		public CWindows.OSPlatformId OSPlatformId {
			get { return _OSPlatformId; }
			set {
				CheckLock("OSPlatformId");

				_OSPlatformId = value;
			}
		}

		public int OSMajorVersion {
			get { return _MajorVersion; }
			set {
				CheckLock("MajorVersion");

				_MajorVersion = value;
			}
		}

		public int OSMinorVersion {
			get { return _MinorVersion; }
			set {
				CheckLock("MinorVersion");

				_MinorVersion = value;
			}
		}

		public int BuildNumber {
			get { return _BuildNumber; }
			set {
				CheckLock("BuildNumber");

				_BuildNumber = value;
			}
		}

		public string OSCSDVersion {
			get { return _CSDVersion; }
			set {
				CheckLock("CSDVersion");

				_CSDVersion = value;
			}
		}
		//-----------------------------------------------------------------------------
		// Extended Properties

		public CWindows.OSSuites OSSuiteFlags {
			get {
				CheckExtendedProperty("OSSuiteFlags");

				return _OSSuiteFlags;
			}

			set {
				CheckLock("OSSuiteFlags");

				_OSSuiteFlags = value;
			}
		}

		public CWindows.OSProductType OSProductType {
			get {
				CheckExtendedProperty("OSProductType");

				return _OSProductType;
			}

			set {
				CheckLock("OSProductType");

				_OSProductType = value;
			}
		}

		public Int16 OSServicePackMajor {
			get {
				CheckExtendedProperty("ServicePackMajor");

				return _ServicePackMajor;
			}

			set {
				CheckLock("ServicePackMajor");

				_ServicePackMajor = value;
			}
		}

		public Int16 OSServicePackMinor {
			get {
				CheckExtendedProperty("ServicePackMinor");

				return _ServicePackMinor;
			}

			set {
				CheckLock("ServicePackMinor");

				_ServicePackMinor = value;
			}
		}

		public byte OSReserved {
			get {
				CheckExtendedProperty("Reserved");

				return _Reserved;
			}

			set {
				CheckLock("Reserved");

				_Reserved = value;
			}
		}

		//-----------------------------------------------------------------------------
		// Get Properties

		public int Platform {
			get { return Conversion.Fix(_OSPlatformId); }
		}

		public int SuiteMask {
			get {
				CheckExtendedProperty("SuiteMask");

				return Conversion.Fix(_OSSuiteFlags);
			}
		}

		public byte ProductType {
			get {
				CheckExtendedProperty("ProductType");

				return (byte)_OSProductType;
			}
		}

		//-----------------------------------------------------------------------------
		// Calculated Properties

		public System.Version Version {
			get {
				if (OSMajorVersion < 0 || OSMinorVersion < 0) {
					return new Version();
				}
				if (BuildNumber < 0) {
					return new Version(OSMajorVersion, OSMinorVersion);
				}
				return new Version(OSMajorVersion, OSMinorVersion, BuildNumber);
			}
		}

		public string VersionString {
			get { return Version.ToString(); }
		}

		public string OSPlatformIdString {
			get {
				switch (OSPlatformId) {
					case CWindows.OSPlatformId.Win32s:
						return "Windows 32s";
					case CWindows.OSPlatformId.Win32Windows:
						return "Windows 32";
					case CWindows.OSPlatformId.Win32NT:
						return "Windows NT";
					case CWindows.OSPlatformId.WinCE:

						return "Windows CE";
					default:
						//Throw New InvalidOperationException("Invalid OSPlatformId: " + OSPlatformId)
						return null;
				}
			}
		}

		static internal bool OSSuiteFlag(CWindows.OSSuites flags, CWindows.OSSuites test)
		{
			return (flags & test) > 0;
		}
		//OSSuiteFlag

		public string OSSuiteString {
			get {
				string s = string.Empty;

				CWindows.OSSuites flags = OSSuiteFlags;

				if (OSSuiteFlag(flags, CWindows.OSSuites.SmallBusiness)) {
					OSSuiteStringAdd(ref s, "Small Business");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.Enterprise)) {
					switch (OSVersion) {
						case CWindows.OSVersion.WinNT4:
							OSSuiteStringAdd(ref s, "Enterprise");
							break;
						case CWindows.OSVersion.Win2000:
							OSSuiteStringAdd(ref s, "Advanced");
							break;
						case CWindows.OSVersion.Win2003:
							OSSuiteStringAdd(ref s, "Enterprise");
							break;
					}
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.BackOffice)) {
					OSSuiteStringAdd(ref s, "BackOffice");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.Communications)) {
					OSSuiteStringAdd(ref s, "Communications");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.Terminal)) {
					OSSuiteStringAdd(ref s, "Terminal Services");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.SmallBusinessRestricted)) {
					OSSuiteStringAdd(ref s, "Small Business Restricted");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.EmbeddedNT)) {
					OSSuiteStringAdd(ref s, "Embedded");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.Datacenter)) {
					OSSuiteStringAdd(ref s, "Datacenter");
				}
				//				if ( OSSuiteFlag( flags, CWindows.OSSuites.SingleUserTS ) )
				//					OSSuiteStringAdd( ref s, "Single User Terminal Services" );
				if (OSSuiteFlag(flags, CWindows.OSSuites.Personal)) {
					OSSuiteStringAdd(ref s, "Home Edition");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.Blade)) {
					OSSuiteStringAdd(ref s, "Web Edition");
				}
				if (OSSuiteFlag(flags, CWindows.OSSuites.EmbeddedRestricted)) {
					OSSuiteStringAdd(ref s, "Embedded Restricted");
				}
				return s;
			}
		}

		private static void OSSuiteStringAdd(ref string s, string suite)
		{
			if (s.Length > 0) {
				s += ", ";
			}
			s += suite;
		}
		//OSSuiteStringAdd

		public string OSProductTypeString {
			get {
				switch (OSProductType) {
					case CWindows.OSProductType.Workstation:

						switch (OSVersion) {
							case CWindows.OSVersion.Win32s:
								return string.Empty;
							case CWindows.OSVersion.Win95:
								return string.Empty;
							case CWindows.OSVersion.Win98:
								return string.Empty;
							case CWindows.OSVersion.WinME:
								return string.Empty;
							case CWindows.OSVersion.WinNT351:
								return string.Empty;
							case CWindows.OSVersion.WinNT4:
								return "Workstation";
							case CWindows.OSVersion.Win2000:
								return "Professional";
							case CWindows.OSVersion.WinXP:
								if (OSSuiteFlag(OSSuiteFlags, CWindows.OSSuites.Personal)) {
									return "Home Edition";
								}
								else {
									return "Professional";
								}

								break;
							case CWindows.OSVersion.Win2003:
								return string.Empty;
							case CWindows.OSVersion.WinVista:
								return string.Empty;
							case CWindows.OSVersion.WinCE:
								return string.Empty;
							default:
								return "Unkown Windows ProductType";
						}

						break;
					case CWindows.OSProductType.DomainController:
						string s = OSSuiteString;

						if (s.Length > 0) {
							s += " ";
						}


						return s + "Domain Controller";
					case CWindows.OSProductType.Server:
						string s = OSSuiteString;

						if (s.Length > 0) {
							s += " ";
						}


						return s + "Server";
					default:
						return "Unkown Windows ProductType";
				}
			}
		}

		public CWindows.OSVersion OSVersion {
			get {
				switch (OSPlatformId) {
					case CWindows.OSPlatformId.Win32s:

						return CWindows.OSVersion.Win32s;
					case CWindows.OSPlatformId.Win32Windows:

						switch (OSMinorVersion) {
							case MinorVersionConst.Win95:
								return CWindows.OSVersion.Win95;
							case MinorVersionConst.Win98:
								return CWindows.OSVersion.Win98;
							case MinorVersionConst.WinME:
								return CWindows.OSVersion.WinME;
							default:
								return CWindows.OSVersion.Unknown;
						}

						break;
					case CWindows.OSPlatformId.Win32NT:

						switch (OSMajorVersion) {
							case MajorVersionConst.WinNT351:
								return CWindows.OSVersion.WinNT351;
							case MajorVersionConst.WinNT4:

								return CWindows.OSVersion.WinNT4;
							case MajorVersionConst.WinNT5:

								switch (OSMinorVersion) {
									case MinorVersionConst.Win2000:
										return CWindows.OSVersion.Win2000;
									case MinorVersionConst.WinXP:
										return CWindows.OSVersion.WinXP;
									case MinorVersionConst.Win2003:
										return CWindows.OSVersion.Win2003;
									default:
										return CWindows.OSVersion.Unknown;
								}

								break;
							case MajorVersionConst.WinNT6:

								switch (OSMinorVersion) {
									case MinorVersionConst.WinVista:
										return CWindows.OSVersion.WinVista;
								}
								break;
							default:
								return CWindows.OSVersion.Unknown;
						}

						break;
					case CWindows.OSPlatformId.WinCE:

						return CWindows.OSVersion.WinCE;
					default:
						return CWindows.OSVersion.Unknown;
				}
			}
		}

		public string OSVersionString {
			get {
				switch (OSVersion) {
					case CWindows.OSVersion.Win32s:
						return "Windows 32s";
					case CWindows.OSVersion.Win95:
						return "Windows 95";
					case CWindows.OSVersion.Win98:
						return "Windows 98";
					case CWindows.OSVersion.WinME:
						return "Windows ME";
					case CWindows.OSVersion.WinNT351:
						return "Windows NT 3.51";
					case CWindows.OSVersion.WinNT4:
						return "Windows NT 4";
					case CWindows.OSVersion.Win2000:
						return "Windows 2000";
					case CWindows.OSVersion.WinXP:
						return "Windows XP";
					case CWindows.OSVersion.Win2003:
						return "Windows 2003";
					case CWindows.OSVersion.WinVista:
						return "Windows Vista";
					case CWindows.OSVersion.WinCE:

						return "Windows CE";
					default:
						//Throw New InvalidOperationException("Invalid OSVersion: " + OSVersion)
						return null;
				}
			}
		}

		//-----------------------------------------------------------------------------
		// State Properties

		public bool ExtendedPropertiesAreSet {
			get { return _ExtendedPropertiesAreSet; }
			set { _ExtendedPropertiesAreSet = value; }
		}

		public bool IsLocked {
			get { return _Locked; }
		}

		public void Lock()
		{
			_Locked = true;
		}
		//Lock

		//-----------------------------------------------------------------------------
		// Property helpers
		private void CheckExtendedProperty(string property)
		{
			if (_ExtendedPropertiesAreSet) {
				return;
			}
			//Throw New InvalidOperationException("'" + [property] + "' is not set")
		}
		//CheckExtendedProperty

		private void CheckLock(string property)
		{
			if (!_Locked) {
				return;
			}
			throw new InvalidOperationException("Cannot set '" + property + "' on locked instance");
		}
		//CheckLock

		//-----------------------------------------------------------------------------
		// Constructors
		public OSVersionInfo()
		{
		}
		//New

		private OSVersionInfo(OSPlatformId osPlatformId)
		{
			_OSPlatformId = osPlatformId;
		}
		//New

		private OSVersionInfo(OSPlatformId osPlatformId, bool locked)
		{
			_OSPlatformId = osPlatformId;

			_Locked = locked;
		}
		//New

		private OSVersionInfo(OSPlatformId osPlatformId, int majorVersion, int minorVersion)
		{
			_OSPlatformId = osPlatformId;
			_MajorVersion = majorVersion;
			_MinorVersion = minorVersion;
		}
		//New

		private OSVersionInfo(OSPlatformId osPlatformId, int majorVersion, int minorVersion, bool locked)
		{
			_OSPlatformId = osPlatformId;
			_MajorVersion = majorVersion;
			_MinorVersion = minorVersion;

			_Locked = locked;
		}
		//New

		public override int GetHashCode()
		{
			return base.GetHashCode();
		}
		//GetHashCode

		public override string ToString()
		{
			string s = OSVersionString;

			if (ExtendedPropertiesAreSet) {
				s += " " + OSProductTypeString;
			}
			if (OSCSDVersion.Length > 0) {
				s += " " + OSCSDVersion;
			}
			s += " v" + VersionString;

			return s;
		}
		//ToString

	}
	//OSVersionInfo 
	//-----------------------------------------------------------------------------
	// OSVersionInfo
	//-----------------------------------------------------------------------------
	// OperatingSystemVersion
	public class OperatingSystemVersion : CWindows.OSVersionInfo
	{

		[DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
		private static extern bool GetVersionEx(		[In(), Out()]
OSVERSIONINFO osVersionInfo);

		[DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
		private static extern bool GetVersionEx(		[In(), Out()]
OSVERSIONINFOEX osVersionInfoEx);
		[DllImport("user32", CharSet = CharSet.Ansi, SetLastError = true, ExactSpelling = true)]
		private static extern int GetSystemMetrics(int nIndex);
		[DllImport("kernel32", CharSet = CharSet.Auto, SetLastError = true, ExactSpelling = true)]
		public static extern bool GetVolumeInformation(string lpRootPathName, StringBuilder lpVolumeNameBuffer, int nVolumeNameSize, ref int lpVolumeSerialNumber, ref int lpMaximumComponentLength, ref int lpFileSystemFlags, StringBuilder lpFileSystemNameBuffer, ref int nFileSystemNameSize);



		//XXXXXX Windows Plus Version XXXXX

		public bool IsWinNT4Plus()
		{
			OperatingSystemVersion Host = new OperatingSystemVersion();
			return (Host.OSPlatformId == OSPlatformId.Win32NT) & (Host.Version.Major >= 4);
		}

		public bool IsWin2000Plus()
		{
			OperatingSystemVersion Host = new OperatingSystemVersion();
			return (Host.OSPlatformId == OSPlatformId.Win32NT) & (Host.Version.Major == 5 & Host.Version.Minor >= 0);
		}

		public bool IsWinXPPlus()
		{
			OperatingSystemVersion Host = new OperatingSystemVersion();
			return (Host.OSPlatformId == OSPlatformId.Win32NT) & (Host.Version.Major >= 5 & Host.Version.Minor >= 1);
		}

		public bool IsWinVistaPlus()
		{
			OperatingSystemVersion Host = new OperatingSystemVersion();
			return (Host.OSPlatformId == OSPlatformId.Win32NT) & (Host.Version.Major >= 6);
		}

		//XXXXX Server Indformation XXXXX

		public bool IsBackOfficeServer()
		{
			bool functionReturnValue = false;
			if (IsWinNT4Plus()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_BACKOFFICE);
			}
			return functionReturnValue;
		}

		public bool IsBladeServer()
		{
			bool functionReturnValue = false;
			OperatingSystemVersion Host = new OperatingSystemVersion();
			if (IsWin2003Server()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_BLADE);
			}
			return functionReturnValue;
		}

		public bool IsDomainController()
		{
			bool functionReturnValue = false;
			//Returns True if the server is a domain
			//controller (Win 2000 or later), including
			//under active directory
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWin2000Server()) {
				functionReturnValue = (OSProductType == VerProductType.VER_NT_SERVER) & (OSProductType == VerProductType.VER_NT_DOMAIN_CONTROLLER);
			}
			return functionReturnValue;
		}

		public bool IsEnterpriseServer()
		{
			bool functionReturnValue = false;
			//Returns True if Windows NT 4.0 Enterprise Edition,
			//Windows 2000 Advanced Server, or Windows Server 2003
			//Enterprise Edition is installed.
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWinNT4Plus()) {
				functionReturnValue = (OSProductType == VerProductType.VER_NT_SERVER) & (bool)(SuiteMask & VerSuiteMask.VER_SUITE_ENTERPRISE);
			}
			return functionReturnValue;
		}

		public bool IsSmallBusinessServer()
		{
			bool functionReturnValue = false;
			//Returns True if Microsoft Small Business Server is installed
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWinNT4Plus()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_SMALLBUSINESS);
			}
			return functionReturnValue;
		}

		public bool IsSmallBusinessRestrictedServer()
		{
			bool functionReturnValue = false;
			//Returns True if Microsoft Small Business Server
			//is installed with the restrictive client license
			//in force
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWinNT4Plus()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_SMALLBUSINESS_RESTRICTED);
			}
			return functionReturnValue;
		}

		public bool IsTerminalServer()
		{
			bool functionReturnValue = false;
			//Returns True if Terminal Services is installed
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWinNT4Plus()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_TERMINAL);
			}
			return functionReturnValue;
		}

		//XXXXXX Windows Operatong Systems XXXXXX

		public bool IsWin95()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_WINDOWS) & (Version.Major == 4 & Version.Minor == 0) & (BuildNumber == 950);
		}

		public bool IsWin95OSR2()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_WINDOWS) & (Version.Major == 4 & Version.Minor == 0) & (BuildNumber == 1111);
		}

		public bool IsWin98()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_WINDOWS) & (Version.Major == 4 & Version.Minor == 10) & (BuildNumber >= 1998);
		}

		public bool IsWinME()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_WINDOWS) & (Version.Major == 4 & Version.Minor == 90) & (BuildNumber >= 3000);
		}

		public bool IsWinNT3()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 3 & Version.Minor == 0);
		}

		public bool IsWinNT31()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 3 & Version.Minor == 1);
		}

		public bool IsWinNT35()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 3 & Version.Minor == 5);
		}

		public bool IsWinNT351()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 3 & Version.Minor == 51);
		}

		public bool IsWinNT4()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 4 & Version.Minor == 0) & (BuildNumber >= 1381);
		}

		public bool IsWinNT4Workstation()
		{
			bool functionReturnValue = false;
			//returns True if running Windows NT4 Workstation
			if (IsWinNT4()) {
				functionReturnValue = (bool)(OSProductType & VerProductType.VER_NT_WORKSTATION);
			}
			return functionReturnValue;
		}

		public bool IsWinNT4Server()
		{
			bool functionReturnValue = false;
			//Dim Sys As OperatingSystemVersion
			if (IsWinNT4()) {
				functionReturnValue = (bool)(OSProductType & VerProductType.VER_NT_SERVER);
			}
			return functionReturnValue;
		}

		public bool IsWin2000()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 5 & Version.Minor == 0) & (BuildNumber >= 2195);
		}

		public bool IsWin2000AdvancedServer()
		{
			bool functionReturnValue = false;
			if (IsWin2000Plus()) {
				functionReturnValue = ((OSProductType == VerProductType.VER_NT_SERVER) | (OSProductType == VerProductType.VER_NT_DOMAIN_CONTROLLER)) & (bool)(SuiteMask & VerSuiteMask.VER_SUITE_ENTERPRISE);
			}
			return functionReturnValue;
		}

		public bool IsWin2000Server()
		{
			bool functionReturnValue = false;
			if (IsWin2000()) {
				functionReturnValue = (OSProductType == VerProductType.VER_NT_SERVER);
			}
			return functionReturnValue;
		}

		public bool IsWin2000Workstation()
		{
			bool functionReturnValue = false;
			if (IsWin2000()) {
				functionReturnValue = (bool)(OSProductType & VerProductType.VER_NT_WORKSTATION);
			}
			return functionReturnValue;
		}

		public bool IsWin2003Server()
		{
			//returns True if running Windows 2003 (.NET) Server
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 5 & Version.Minor == 2) & (BuildNumber == 3790);
		}

		public bool IsWin2003ServerR2()
		{
			bool functionReturnValue = false;
			//returns True if running
			//Windows 2003 (.NET) Server Release 2
			if (IsWin2003Server()) {
				functionReturnValue = (bool)GetSystemMetrics(VerSuiteMask.VER_SM_SERVERR2);
			}
			return functionReturnValue;
		}

		public bool IsWinXP()
		{
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 5 & Version.Minor == 1) & (BuildNumber >= 2600);
		}

		public bool IsWinXPSP2()
		{
			bool functionReturnValue = false;
			//returns True if running Windows XP SP2 (Service Pack 2)
			if (IsWinXP()) {
				functionReturnValue = Strings.InStr(OSCSDVersion, "Service Pack 2") > 0;
			}
			return functionReturnValue;
		}

		public bool IsWinXPMediaCenter()
		{
			bool functionReturnValue = false;
			//returns True if running Windows XP Media Centre
			if (IsWinXP()) {
				functionReturnValue = (bool)GetSystemMetrics(VerSuiteMask.VER_SM_MEDIACENTER);
			}
			return functionReturnValue;
		}

		public bool IsWinXPHomeEdition()
		{
			bool functionReturnValue = false;
			//returns True if running Windows XP Home Edition
			if (IsWinXP()) {
				functionReturnValue = ((SuiteMask & VerSuiteMask.VER_SUITE_PERSONAL) == VerSuiteMask.VER_SUITE_PERSONAL);
			}
			return functionReturnValue;
		}

		public bool IsWinXPProEdition()
		{
			bool functionReturnValue = false;
			//returns True if running WinXP Pro
			if (IsWinXP()) {
				functionReturnValue = !((SuiteMask & VerSuiteMask.VER_SUITE_PERSONAL) == VerSuiteMask.VER_SUITE_PERSONAL);
			}
			return functionReturnValue;
		}

		//Returns False positive?
		public bool IsWinXPStarter()
		{
			bool functionReturnValue = false;
			//returns True if running Windows XP Starter
			if (IsWinXP()) {
				functionReturnValue = (bool)GetSystemMetrics(VerSuiteMask.VER_SM_STARTER);
			}
			return functionReturnValue;
		}

		//Returns False positive?
		public bool IsWinXPTabletPc()
		{
			bool functionReturnValue = false;
			//'returns True if running Windows XP Tablet Pc
			if (IsWinXP()) {
				functionReturnValue = (bool)GetSystemMetrics(VerSuiteMask.VER_SM_TABLETPC);
			}
			return functionReturnValue;
		}

		public bool IsWinXPEmbedded()
		{
			bool functionReturnValue = false;
			//Returns True if OS is Windows XP Embedded
			//OSVERSIONINFOEX supported on NT4 or
			//later only, so a test is required
			//before using
			if (IsWinXP()) {
				functionReturnValue = (bool)(SuiteMask & VerSuiteMask.VER_SUITE_EMBEDDEDNT);
			}
			return functionReturnValue;
		}

		//Left out is needed will need to be added

		public bool IsWinXP64()
		{
		}

		public bool IsWinVista()
		{
			//returns True if running Windows Vista
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 6);
		}

		public bool IsWinLonghornServer()
		{
			//returns True if running Windows Longhorn Server
			return (OSPlatformId == VerSuiteMask.VER_PLATFORM_WIN32_NT) & (Version.Major == 6 & Version.Minor == 0) & (OSProductType != VerProductType.VER_NT_WORKSTATION);
		}

		//-----------------------------------------------------------------------------
		// Interop Types
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
		private class OSVERSIONINFO
		{
			public int OSVersionInfoSize;
			public int MajorVersion;
			public int MinorVersion;
			public int BuildNumber;
			public int PlatformId;
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 0x80)]

			public string CSDVersion;
			public OSVERSIONINFO()
			{
				OSVersionInfoSize = Marshal.SizeOf(this);
			}
			//New

			private void StopTheCompilerComplaining()
			{
				MajorVersion = 0;
				MinorVersion = 0;
				BuildNumber = 0;
				PlatformId = 0;
				CSDVersion = string.Empty;
			}
			//StopTheCompilerComplaining
		}
		//OSVERSIONINFO

		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
		private class OSVERSIONINFOEX
		{
			public int OSVersionInfoSize;
			public int MajorVersion;
			public int MinorVersion;
			public int BuildNumber;
			public int PlatformId;
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 0x80)]
			public string CSDVersion;
			public Int16 ServicePackMajor;
			public Int16 ServicePackMinor;
				//UInt16
			public Int16 SuiteMask;
			public byte ProductType;

			public byte Reserved;
			public OSVERSIONINFOEX()
			{
				OSVersionInfoSize = Marshal.SizeOf(this);
			}
			//New

			private void StopTheCompilerComplaining()
			{
				MajorVersion = 0;
				MinorVersion = 0;
				BuildNumber = 0;
				PlatformId = 0;
				CSDVersion = string.Empty;
				ServicePackMajor = 0;
				ServicePackMinor = 0;
				//SuiteMask = 0
				ProductType = 0;
				Reserved = 0;
			}
			//StopTheCompilerComplaining
		}
		//OSVERSIONINFOEX
		//-----------------------------------------------------------------------------
		// Interop constants
		private class VerPlatformId
		{
			public const Int32 Win32s = 0;
			public const Int32 Win32Windows = 1;
			public const Int32 Win32NT = 2;

			public const Int32 WinCE = 3;
			private VerPlatformId()
			{
			}
			//New
		}
		//VerPlatformId 

		private class VerSuiteMask
		{
			//--------------------------------------------------------------^--- Specified cast is not valid.
			public const int VER_PLATFORM_WIN32s = 0;
			public const int VER_PLATFORM_WIN32_WINDOWS = 1;
			public const int VER_PLATFORM_WIN32_NT = 2;
			public const long VER_SM_MEDIACENTER = 87;
			public const long VER_SM_SERVERR2 = 89;
			public const long VER_SM_TABLETPC = 86;
			public const long VER_SM_STARTER = 88;
			public const int VER_WORKSTATION_NT = 0x40000000;
			public const int VER_SUITE_SMALLBUSINESS = 0x1;
			public const int VER_SUITE_ENTERPRISE = 0x2;
			public const int VER_SERVER_NT = 0x3;
			public const int VER_SUITE_BACKOFFICE = 0x4;
			public const int VER_SUITE_COMMUNICATIONS = 0x8;
			public const int VER_SUITE_TERMINAL = 0x10;
			public const int VER_SUITE_SMALLBUSINESS_RESTRICTED = 0x20;
			public const int VER_SUITE_EMBEDDEDNT = 0x40;
			public const int VER_SUITE_DATACENTER = 0x80;
			public const int VER_SUITE_SINGLEUSERTS = 0x100;
			public const int VER_SUITE_PERSONAL = 0x200;
			public const int VER_SUITE_BLADE = 0x400;
			public const int VER_SUITE_EMBEDDED_RESTRICTED = 0x800;

			public const long PROCESSOR_ARCHITECTURE_IA64 = 6;
			private VerSuiteMask()
			{
			}
			//New
		}
		//VerSuiteMask 

		private class VerProductType
		{
			public const byte VER_NT_WORKSTATION = 0x1;
			public const byte VER_NT_DOMAIN_CONTROLLER = 0x2;

			public const byte VER_NT_SERVER = 0x3;
			private VerProductType()
			{
			}
			//New
		}
		//VerProductType 
		//-----------------------------------------------------------------------------
		// Interop methods
		private class NativeMethods
		{
			private NativeMethods()
			{
			}
			//New

			[DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
			public static extern bool GetVersionEx(			[In(), Out()]
OSVERSIONINFO osVersionInfo);

			[DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
			public static extern bool GetVersionEx(			[In(), Out()]
OSVERSIONINFOEX osVersionInfoEx);

		}
		//NativeMethods

		//-----------------------------------------------------------------------------
		// Constructors
		internal OperatingSystemVersion()
		{
			OSVERSIONINFO osVersionInfo = new OSVERSIONINFO();

			if (!UseOSVersionInfoEx(osVersionInfo)) {
				InitOsVersionInfo(osVersionInfo);
			}
			else {
				InitOsVersionInfoEx();
			}
		}
		//New

		// check for NT4 SP6 or later
		private static bool UseOSVersionInfoEx(OSVERSIONINFO info)
		{
			bool b = NativeMethods.GetVersionEx(info);

			//If Not b Then
			//    Dim [error] As Integer = Marshal.GetLastWin32Error()

			//    Throw New InvalidOperationException("Failed to get OSVersionInfo. Error = 0x" + [error].ToString("8X", CultureInfo.CurrentCulture))
			//End If

			if (info.MajorVersion < 4) {
				return false;
			}
			if (info.MajorVersion > 4) {
				return true;
			}
			if (info.MinorVersion < 0) {
				return false;
			}
			if (info.MinorVersion > 0) {
				return true;
			}
			// TODO: CSDVersion for NT4 SP6
			if (info.CSDVersion == "Service Pack 6") {
				return true;
			}
			return false;
		}
		//UseOSVersionInfoEx

		private void InitOsVersionInfo(OSVERSIONINFO info)
		{
			OSPlatformId = GetOSPlatformId(info.PlatformId);

			OSMajorVersion = info.MajorVersion;
			OSMinorVersion = info.MinorVersion;
			BuildNumber = info.BuildNumber;
			//			PlatformId     = info.PlatformId   ;
			OSCSDVersion = info.CSDVersion;
		}
		//InitOsVersionInfo

		private void InitOsVersionInfoEx()
		{
			OSVERSIONINFOEX info = new OSVERSIONINFOEX();

			bool b = NativeMethods.GetVersionEx(info);

			//If Not b Then
			//   Dim [error] As Integer = Marshal.GetLastWin32Error()

			//   Throw New InvalidOperationException("Failed to get OSVersionInfoEx. Error = 0x" + [error].ToString("8X", CultureInfo.CurrentCulture))
			//End If

			OSPlatformId = GetOSPlatformId(info.PlatformId);

			OSMajorVersion = info.MajorVersion;
			OSMinorVersion = info.MinorVersion;
			BuildNumber = info.BuildNumber;
			//			PlatformId         = info.PlatformId       ;
			OSCSDVersion = info.CSDVersion;

			OSSuiteFlags = GetOSSuiteFlags(info.SuiteMask);
			OSProductType = GetOSProductType(info.ProductType);

			OSServicePackMajor = info.ServicePackMajor;
			OSServicePackMinor = info.ServicePackMinor;
			//			SuiteMask          = info.SuiteMask        ;
			//			ProductType        = info.ProductType      ;
			OSReserved = info.Reserved;

			ExtendedPropertiesAreSet = true;
		}
		//InitOsVersionInfoEx

		private static CWindows.OSPlatformId GetOSPlatformId(int platformId)
		{
			switch (platformId) {
				case VerPlatformId.Win32s:
					return CWindows.OSPlatformId.Win32s;
				case VerPlatformId.Win32Windows:
					return CWindows.OSPlatformId.Win32Windows;
				case VerPlatformId.Win32NT:
					return CWindows.OSPlatformId.Win32NT;
				case VerPlatformId.WinCE:

					return CWindows.OSPlatformId.WinCE;
				//Case Else
				//Throw New InvalidOperationException("Invalid PlatformId: " + platformId)
			}
		}
		//GetOSPlatformId

		private static CWindows.OSSuites GetOSSuiteFlags(Int16 suiteMask)
		{
			return (CWindows.OSSuites)suiteMask;
		}
		//GetOSSuiteFlags

		private static CWindows.OSProductType GetOSProductType(byte productType)
		{
			switch (productType) {
				case VerProductType.VER_NT_WORKSTATION:
					return CWindows.OSProductType.Workstation;
				case VerProductType.VER_NT_DOMAIN_CONTROLLER:
					return CWindows.OSProductType.DomainController;
				case VerProductType.VER_NT_SERVER:
					return CWindows.OSProductType.Server;
				//Case Else
				//Throw New InvalidOperationException("Invalid ProductType: " + productType)
			}
		}
		//GetOSProductType

		public string WinDir()
		{
			string WinSysPath = System.Environment.GetFolderPath(Environment.SpecialFolder.System);
			return WinSysPath.Substring(0, WinSysPath.LastIndexOf("\\"));
		}

		//Purpose to return the windows system directory
		public string WinSysDir()
		{
			return System.Environment.SystemDirectory;
		}

		//Purpose to return the drive type eg NTFS, Fat32 etc
		public string SystemType()
		{
			const int StringBufferLength = 255;
			string lsRootPathName = IO.Directory.GetDirectoryRoot(Application.StartupPath);
			StringBuilder lsFileSystemNameBuffer = new StringBuilder(StringBufferLength);
			GetVolumeInformation(lsRootPathName, null, null, ref null, ref null, ref null, lsFileSystemNameBuffer, ref null);
			return lsFileSystemNameBuffer.ToString();
		}

		//Purpose to send a true value if drive type is NTFS
		public bool IsNTFSDrive()
		{
			if (SystemType().ToString() == "NTFS") return true; 
		}

	}
	//OperatingSystemVersion 
}
//Common '-----------------------------------------------------------------------------
// OperatingSystemVersion
//-----------------------------------------------------------------------------
